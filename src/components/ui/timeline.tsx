        ...
        {data.map((item: TimelineEntry, index: number) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-lg md:pl-20 font-mono font-medium uppercase">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-sm font-mono mb-4 text-left font-medium uppercase text-foreground">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
